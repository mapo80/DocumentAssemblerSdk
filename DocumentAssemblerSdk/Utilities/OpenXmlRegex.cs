...
namespace DocumentAssembler.Core
{
    public class OpenXmlRegex
    {
        ...
        {
            return ReplaceInternal(content, regex, null, null, false, string.Empty, true);
        }
        ...
        {
            return ReplaceInternal(content, regex, replacement, doReplacement, false, string.Empty, true);
        }
        ...
        {
            return ReplaceInternal(content, regex, replacement, doReplacement, false, string.Empty, coalesceContent);
        }
                        ...
                        if (trackRevisions)
                        {
                            if (replacement != "")
                            {
                                ...
                                var newIns = new XElement(W.ins,
                                    new XAttribute(W.author, revisionTrackingAuthor ?? string.Empty),
            ...
            Func<XElement, Match, bool>? callback, ReplaceInternalInfo counter)
        {
            ...
            if (element.Name == A.p)
            {
                ...
                if (replacement == null)
                {
                    foreach (var match in matchCollection.Cast<Match>())
                    {
                        callback?.Invoke(paragraph, match);
                    }
                }
                else
                {
